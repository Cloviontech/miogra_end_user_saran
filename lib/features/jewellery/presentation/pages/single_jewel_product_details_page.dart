...
class JewelProductDetailsPage extends StatefulWidget {
  ...
  const JewelProductDetailsPage({super.key,  this.productId,  this.shopId, this.link, this.subCategory});

  @override
  State<JewelProductDetailsPage> createState() => _JewelProductDetailsPageState();
}

class _JewelProductDetailsPageState extends State<JewelProductDetailsPage> {
 ...
 String userId = 'a';
  ...
  bool loadingFetchSingleJewelproduct= true;



  
Future<List<AllJewelproducts>> fetchSingleJewelProduct(String shopId , String productId) async {
  final response = await http.get(Uri.parse(
      // 'http://${ApiServices.ipAddress}/get_single_shopproduct/$shopId/$productId'));
      'http://${ApiServices.ipAddress}/single_jewelproduct/$shopId/$productId'));
  ...
  if (response.statusCode == 200) {
    ...
    return jsonResponse.map((json) => AllJewelproducts.fromJson(json)).toList();
  } else {
  ...
  late Future<List<AllJewelproducts>> futurefetchSingleJewelproduct;
...
// :
 ...
 FutureBuilder<List<AllJewelproducts>>(
                  future: futurefetchSingleJewelproduct,
                  builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return Center(child: CircularProgressIndicator());
          } else if (snapshot.hasError) {
