...
class FreshCutProductDetailsPage extends StatefulWidget {
  ...
  const FreshCutProductDetailsPage({super.key,  this.productId,  this.shopId, this.link, this.subCategory});

  @override
  State<FreshCutProductDetailsPage> createState() => _FreshCutProductDetailsPageState();
}

class _FreshCutProductDetailsPageState extends State<FreshCutProductDetailsPage> {
 ...
 String userId = 'a';
  ...
  addToCart(String productId, String subCategory, int quantity) async {
   ...
   List < AllFreshcutproducts> singleFreshcutproducts = [] ;

  bool loadingFetchSingleFreshcutproducts = true;

  Future<List < AllFreshcutproducts>> fetchSingleFreshcutproducts() async {
    final response = await http
        .get(Uri.parse('http://${ApiServices.ipAddress}/single_freshproduct/${widget.shopId}/${widget.shopId}'));
debugPrint(response.statusCode.toString());
    if (response.statusCode == 200) {
      List<dynamic> jsonResponse = json.decode(response.body);

      setState(() {
         loadingFetchSingleFreshcutproducts = false;
      });
        // singleFreshcutproducts = 
...
Future<List<SingleFreshproduct>> fetchSingleFreshCutProducts1(String shopId , String productId) async {
  ...
  late Future<List<SingleFreshproduct>> futureSingleFreshcutproducts;
...
futureSingleFreshcutproducts =  fetchSingleFreshCutProducts1(widget.shopId.toString(), widget.productId.toString())  ;
            ...
            Expanded(
                child: GestureDetector(
                  onTap: (){
                    ...
                    YourOrderPage(shopId: widget.shopId, productId: widget.productId,)
...
// :
 ...
 FutureBuilder<List<SingleFreshproduct>>(
                  future: futureSingleFreshcutproducts,
                  builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return Center(child: CircularProgressIndicator());
          } else if (snapshot.hasError) {
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                   Padding(
                    padding: EdgeInsets.symmetric(horizontal: 10),
                    child: Text(
                  "Seller ID : ${snapshot.data![0].freshId}",
                      style: TextStyle(color: Colors.grey),
                    ),
                  ),
                  const SizedBox(height: 10),
            ...
            GestureDetector(
              onTap: () {
                ...
                bottomDetailsScreen(context: context, left: ["first", "second", 'Third'], right: ['value1', 'value2', 'value3']);
